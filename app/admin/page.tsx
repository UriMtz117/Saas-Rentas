...
export default async function AdminDashboard({ 
  ...
  // 1. VALIDACIÓN DE SEGURIDAD MEDIANTE COOKIES
  const cookieStore = await cookies();
  const uid = cookieStore.get("userId")?.value;

  if (!uid) redirect("/login");

  // Verificamos en la BD que el usuario realmente sea ADMIN
  const adminCheck = await prisma.usuario.findUnique({
    where: { id: uid },
    select: { rol: true }
  });

  if (adminCheck?.rol !== "ADMIN") {
    redirect("/dashboard"); // Si no es admin, fuera de aquí
  }

  // 2. LÓGICA DE FILTROS (Vía URL SearchParams)
  ...
  return (
    <div className="p-6 md:p-12 bg-[#f8fafc] min-h-screen font-sans text-slate-900 italic-none">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          <div>
            ...
            <h1 className="text-5xl font-black text-slate-900 tracking-tighter italic uppercase leading-none">Control Panel</h1>
          </div>
          <div className="bg-white px-6 py-3 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-3">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-ping"></div>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest font-mono">Status: Master Admin</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          ...
          <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm flex flex-col justify-center relative overflow-hidden group">
            <p className="text-slate-400 text-[9px] font-black uppercase tracking-[0.2em] mb-2">Unidades en Red</p>
        ...
        <div className="bg-white rounded-[50px] border border-slate-100 shadow-sm overflow-hidden border-b-8 border-b-blue-600">
          ...
          <div className="p-8 md:p-10 border-b border-slate-50 bg-white/50 backdrop-blur-md">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                    <h3 className="text-2xl font-black text-slate-800 tracking-tight italic flex items-center gap-2 uppercase italic">
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-widest italic">
                        Visualizando: {currentPlan || 'Todos'} / {currentRol || 'Todos'}
                    </p>
                </div>

                {/* BOTONERA DE FILTROS (Sin el UID en el link) */}
                <div className="flex flex-wrap justify-center gap-2">
                    ...
                    <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200/50 shadow-inner">
                        ...
                        <FilterButton href="/admin?rol=USER" active={currentRol === 'USER'} label="Owners" />
                    </div>
                </div>
            </div>
          </div>
                  ...
                  <tr key={u.id} className="border-b border-slate-50 hover:bg-blue-50/30 transition-all group">
                    ...
                    <td className="p-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col">
                                <span className="text-slate-800 font-black uppercase text-base tracking-tighter italic">{u.nombre}</span>
                    ...
                    <td className="p-8 text-right">
                        <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button className="text-red-300 hover:text-red-600"><Trash2 size={20} /></button>
                        </div>
                    </td>
                  </tr>
                ))}
          ...
          {usuarios.length === 0 && (
            <div className="p-32 text-center flex flex-col items-center">
                ...
                <h4 className="text-slate-800 font-black text-2xl tracking-tighter italic uppercase">Sin Coincidencias</h4>
                <Link href="/admin" className="mt-8 text-blue-600 font-black text-xs uppercase tracking-widest border-b-2 border-blue-600 pb-1 hover:text-slate-900 transition-all">Limpiar Búsqueda</Link>
            </div>
          )}
        </div>
