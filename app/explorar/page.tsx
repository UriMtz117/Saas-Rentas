    ...
    <div className="p-4 md:p-8 bg-[#f8fafc] min-h-screen font-sans text-slate-900">
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-12">
            <h1 className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter leading-none italic">Marketplace</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          <aside className="bg-white p-8 rounded-[50px] shadow-sm border border-slate-100 h-fit space-y-10 shadow-2xl shadow-blue-500/5">
            ...
            {/* SECCIÓN: RANGO DE PRECIOS (LO QUE PEDISTE MANTENER) */}
            <div className="space-y-4">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Rango Personalizado ($)</p>
                <div className="flex gap-2">
                    <div className="relative w-1/2">
                        <DollarSign className="absolute left-3 top-3.5 text-slate-300" size={12} />
                        <input placeholder="Min" type="number" value={filtros.min} onChange={(e)=>setFiltros({...filtros, min: e.target.value, page: 1})} className="w-full pl-7 p-3 bg-slate-50 rounded-xl outline-none text-xs font-bold border border-transparent focus:border-blue-500 transition-all" />
                    </div>
                    <div className="relative w-1/2">
                        <DollarSign className="absolute left-3 top-3.5 text-slate-300" size={12} />
                        <input placeholder="Max" type="number" value={filtros.max} onChange={(e)=>setFiltros({...filtros, max: e.target.value, page: 1})} className="w-full pl-7 p-3 bg-slate-50 rounded-xl outline-none text-xs font-bold border border-transparent focus:border-blue-500 transition-all" />
                    </div>
                </div>
            </div>
            ...
            {/* SECCIÓN: CATEGORÍA */}
            <div className="space-y-4">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Tipo de Propiedad</p>
            ...
            <button onClick={() => setFiltros({q:'', tipo:'TODOS', min:'', max:'', sort:'newest', page: 1})} className="w-full py-4 text-slate-300 font-black text-[9px] uppercase hover:text-red-500 transition-all border-t border-slate-50 pt-6 italic tracking-widest">Limpiar todo</button>
          </aside>
          ...
          <div className="lg:col-span-3 space-y-8">
            <div className="relative group">
                ...
                <input placeholder="Escribe el nombre de la propiedad o calle..." className="w-full pl-16 pr-8 py-6 bg-white border border-slate-100 rounded-[40px] shadow-2xl outline-none font-bold text-lg focus:ring-4 focus:ring-blue-500/5 transition-all" onChange={(e) => setFiltros({...filtros, q: e.target.value, page: 1})} />
            </div>

            {error && (
                <div className="bg-red-50 text-red-600 p-8 rounded-[40px] border border-red-100 flex items-center gap-4 font-black text-xs uppercase tracking-widest italic animate-pulse"><AlertCircle size={24} /> {error}</div>
            )}
            ...
            {loading ? (
                    ...
                    {data.items.map((p: any) => (
                        <div key={p.id} className="bg-white rounded-[55px] shadow-sm border border-slate-100 overflow-hidden hover:shadow-2xl transition-all duration-700 group flex flex-col relative">
                            ...
                            <div className="p-8 flex-1 flex flex-col justify-between">
                                <div>
                                    <h2 className="text-2xl font-black text-slate-800 uppercase italic tracking-tighter line-clamp-1">{p.nombre}</h2>
                                ...
                                <div className="flex justify-between items-center pt-8 border-t border-slate-50">
                                    <div><p className="text-[9px] font-black text-slate-300 uppercase mb-1">Renta Mensual</p><p className="text-3xl font-black text-blue-600 tracking-tighter italic">${p.precio.toLocaleString()}</p></div>
                                    <Link href={`/propiedades/${p.id}?uid=${localStorage.getItem("userId")}`} className="bg-slate-900 text-white p-4 rounded-2xl hover:bg-blue-600 transition-all shadow-xl"><ChevronRight/></Link>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                {data.totalPages > 1 && (
                    <div className="flex justify-center items-center gap-6 pt-20">
                        <button disabled={filtros.page === 1} onClick={() => setFiltros({...filtros, page: filtros.page - 1})} className="p-5 bg-white border border-slate-100 rounded-3xl disabled:opacity-30 hover:bg-slate-900 hover:text-white transition-all shadow-sm shadow-blue-500/5"><ChevronLeft size={20}/></button>
                        <span className="font-black text-xs uppercase tracking-widest italic">Página {data.page} de {data.totalPages}</span>
                        <button disabled={filtros.page === data.totalPages} onClick={() => setFiltros({...filtros, page: filtros.page + 1})} className="p-5 bg-white border border-slate-100 rounded-3xl disabled:opacity-30 hover:bg-slate-900 hover:text-white transition-all shadow-sm shadow-blue-500/5"><ChevronRight size={20}/></button>
                    </div>
                )}
                </>
            )}
